...
const handleLogin = async (): Promise<void> => {
  ...
  try {
    ...
    if (error.response?.status === 422) {
      ...
      showToast(
        validationErrors.email?.[0] || validationErrors.password?.[0] || 'Validation error',
        'error'
      )
...
<template>
  <div class="flex h-screen flex-col items-center justify-center bg-white space-y-6 px-4">
    <NotepadText class="w-12 h-12 text-black" />

    <Card class="w-full max-w-md">
      <h1 class="text-2xl font-bold text-center mb-2">Sign In</h1>
      <p class="text-gray-500 text-center mb-6">Login to continue using this app</p>
      ...
      <form class="space-y-4" @submit.prevent="handleLogin">
        ...
        <div>
          <div class="flex justify-between items-center mb-1">
            <label :for="passwordInputId" class="block text-sm font-medium text-gray-700">
              Password
            </label>
            <a href="#" class="text-xs text-black hover:underline">Forgot your password?</a>
          </div>
        ...
        <Button type="submit" :disabled="loading">
          <span v-if="loading" class="animate-spin mr-2 border-2 border-t-2 border-gray-200 rounded-full w-4 h-4 inline-block"></span>
          Login
        </Button>
      </form>
    </Card>
