...
import { TaskCard, SideBar, HeaderBar, TaskList, TaskInput, LoadingOverlay, SearchResults } from '~/components/ui'
...
const filteredTasks = computed(() => {
  ...
  const result = allTasks.filter(t => {
    ...
    else key = due.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' ,   year: 'numeric', // ✅ include the year
 })
...
<div v-if="taskStore.searchResults.length > 0 || searchLoading" class="flex flex-col h-full">
  ...
  <div class="flex-1 overflow-y-auto">
    <div class="w-full max-w-3xl mx-auto">
      ...
      <div v-if="searchLoading" class="flex justify-center items-center py-6">
...
<div v-else class="flex flex-col h-full">
      ...
      <!-- Center TaskCard if no tasks -->
<div v-if="filteredTasks.length === 0" class="flex flex-col items-center mt-36">
  <TaskCard
    v-model="newTask"
