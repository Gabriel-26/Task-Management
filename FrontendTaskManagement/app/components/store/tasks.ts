      ...
      tasksThisMonth.forEach((t) => {
        ...
        const dateStr =
          dateObj.toDateString() === today.toDateString()
            ? 'Today'
            : new Date(dateObj.getTime() - 86400000).toDateString() === today.toDateString()
            ? 'Yesterday'
      ...
      return allTasks.filter((t) => {
        ...
        const dateStr =
          due.toDateString() === new Date().toDateString()
            ? 'Today'
            : new Date(due.getTime() - 86400000).toDateString() === new Date().toDateString()
            ? 'Yesterday'
  ...
  actions: {
    async setActiveItem(item: string) {
      ...
      if (targetDate && !this.tasksByDate[targetDate]) {
        console.log('[setActiveItem] fetching tasks for targetDate:', targetDate)
...
async fetchTasks(from?: string, to?: string, force = false) {
  ...
  if (!this.tasksByDate[dateKey] || force) this.tasksByDate[dateKey] = []

  const existingIndex = this.tasksByDate[dateKey].findIndex(e => e.id === t.id)
  if (existingIndex >= 0) {
...
async addTaskAPI(statement: string) {
  ...
  try {
    ...
    this.tasksByDate[key] = [
      ...this.tasksByDate[key].filter((t) => t.id !== tempId && t.id !== savedTask.id),
      savedTask,
    ]
    ...
    // Remove temp task
    const key = normalizeDate(taskDate)
    this.tasksByDate[key] = (this.tasksByDate[key] ?? []).filter((t) => t.id !== tempId)
    localStorage.setItem('tasksByDate', JSON.stringify(this.tasksByDate))
  }
},

async editTaskAPI(taskId: number, newStatement: string) {
  ...
  taskToUpdate.statement = newStatement
 ...
 if (keyFound) {
  this.tasksByDate[keyFound] = this.tasksByDate[keyFound]?.map(t =>
...
localStorage.setItem('tasksByDate', JSON.stringify({ [keyFound]: this.tasksByDate[keyFound] }))
  } catch (err) {
...
async deleteTaskAPI(taskId: number) {
  ...
  // remove locally
  this.tasksByDate[keyFound] = this.tasksByDate[keyFound]?.filter(t => t.id !== taskId) ?? []
...
async updateTaskOrder(taskId: number, newIndex: number) {
  ...
  const tasks = [...(this.tasksByDate[keyFound] ?? [])].sort((a, b) => a.order - b.order)
  ...
  try {
    ...
    const tasks: Task[] = Array.isArray(response.data?.data) ? response.data.data : []
...
async toggleComplete(taskId: number) {
  ...
  taskToUpdate.updated_at = new Date().toISOString()
  console.log('[toggleComplete] optimistic local toggle:', taskToUpdate.completed)
  ...
  try {
    const token = localStorage.getItem('auth_token')
    console.log('[toggleComplete] calling API PATCH /tasks/' + taskId + '/toggle')
...
updateSearchResults(taskId: number, updatedTask: Partial<Task> | null = null) {
