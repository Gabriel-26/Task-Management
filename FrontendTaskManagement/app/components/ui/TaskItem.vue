...
watch(() => props.task, (newVal) => { editText.value = newVal })
...
const cyclePriority = async () => {
  const newPriority = ((props.priority || 3) % 3) + 1 // cycles 1 → 2 → 3 → 1
  saving.value = true
  try {
    await props.editTask(props.id, props.task, newPriority) // send new priority to API
  } finally {
    saving.value = false
  }
}
...
const startEdit = () => { editing.value = true; editText.value = props.task }
const confirmEdit = async () => {
  ...
  saving.value = true
  try { await props.editTask(props.id, editText.value) } 
  finally { saving.value = false; editing.value = false }
}
</script>

<template>
  <div class="flex items-center border justify-between mb-2 p-2 rounded-lg w-full hover:bg-gray-100 transition">
    <!-- Drag handle -->
...
<div class=" flex items-center">
  ...
  ></div>
    ...
    <div class="flex-1 relative">
      <p v-if="!editing" @click="startEdit"
         :class="[props.done ? 'line-through text-gray-400' : 'text-gray-800', 'cursor-text']">
        {{ props.task }}
      </p>
      <div v-else class="flex items-center">
        <input v-model="editText"
               ...
               :disabled="saving"
        />
        <button @click="confirmEdit" class="ml-2 text-blue-500 hover:text-blue-700" :disabled="saving">
          <Check class="w-4 h-4"/>
        </button>
        <div v-if="saving" class="ml-2 w-4 h-4 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"></div>
      </div>
    </div>
