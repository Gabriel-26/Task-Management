...
const formatDate = (date: Date) =>
  date.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric'   ,  year: 'numeric', // ✅ include the year
 })
...
for (let w = currentWeekNumber - 1; w >= 1; w--) {
  ...
  const title = w === currentWeekNumber - 1 ? 'Last week' : `Week ${w} of ${today.toLocaleDateString('en-US', { month: 'long' })}`
...
<p
  ...
  :class="[
    ...
    taskStore.activeItem === item ? 'bg-black text-white' : 'hover:bg-gray-200'
  ]"
>
  {{ item }}
</p>
